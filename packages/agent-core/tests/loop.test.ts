...
describe('composeSkills', () => {
  ...
  it('reflects a skill that changes the tools it offers after composition', async () => {
    // slides withholds its Genspark-only tools until that account is known to
    // be available, which is answered after the panel builds the skill
    let unlocked = false
    const dynamic: AgentSkill = {
      id: 'dyn',
      systemPrompt: '',
      get tools() {
        return unlocked ? [{ name: 'extra', description: '', inputSchema: {} }] : []
      },
      executeTool: () => ({ output: 'from-dyn', summary: 'dyn' }),
    }
    const merged = composeSkills('x', '', [dynamic])
    expect(merged.tools).toEqual([])
    expect((await merged.executeTool({ id: '1', name: 'extra', input: {} })).isError).toBe(true)

    unlocked = true
    expect(merged.tools.map((t) => t.name)).toEqual(['extra'])
    // routing has to follow the tool list, or the model is offered something
    // the composed skill cannot execute
    expect((await merged.executeTool({ id: '2', name: 'extra', input: {} })).output).toBe(
      'from-dyn',
    )
  })
